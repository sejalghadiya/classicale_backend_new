...
export const getAllConversation33 = async (req, res) => {
  ...
  try {
    ...
    conversations.forEach((conversation) => {
      // Add sender details if they are not the logged-in user
      if (conversation.senderId._id.toString() !== userId) {
      ...
      // Add receiver details if they are not the logged-in user
      if (conversation.receiverId._id.toString() !== userId) {
        users.set(conversation.receiverId._id.toString(), {
          userId: conversation.receiverId._id,
          userName: conversation.receiverName,
          userEmail: conversation.receiverEmail,
...
export const getAllConversation11 = async (req, res) => {
  try {
    const conversations = await ConversationModel.find({});

    conversations.forEach((conversation) => {
      // Assuming you're dealing with participants (or similar) that might be null
      if (conversation.participants && conversation.participants.length > 0) {
        conversation.participants.forEach(async (participant) => {
          if (participant) {
            // Check if the participant is not null
            const user = await UserModel.findById(participant._id); // This might return null
            if (user) {
              console.log(user._id); // Only access _id if user is not null
            } else {
              console.error("User not found for participant:", participant._id);
            }
          }
        });
      }
